...
-- NOTE: previous versions of this file created a `public_resenas` view and
-- revoked direct SELECT on the base `resenas` table. That view and the
-- associated revokes were removed to restore the original public read
-- behavior. If you previously applied the SQL from this file to your
-- database, run the DROP/GRANT commands below in the Supabase SQL editor
-- to remove the view and restore SELECT privileges for the `anon` role.

-- SQL to run in Supabase (if needed):
-- DROP VIEW IF EXISTS public_resenas;
